...
@interface CurrentViewController ()<UITextFieldDelegate,UIAlertViewDelegate,ReminderLoginViewDelegate>
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if (![[NSUserDefaults standardUserDefaults] boolForKey:@"hasLogin"]) {
        ...
        _ShouYiLab.text = @"6.0";
        _jcsyLab.text = _tqsyLab.text = @"登陆后查看";
        _jcsyLab.font = _tqsyLab.font = [UIFont systemFontOfSize:14];
        
    }
    else{
        [_JoinButton setTitle:@"加入" forState:UIControlStateNormal];
        _jcsyLab.font = _tqsyLab.font = [UIFont systemFontOfSize:16];
...
//    cardInfor.text = @"招行＊＊8888（储蓄）";
    ...
    UIView *line2 = [[UIView alloc] initWithFrame:CGRectMake(20, _CardInfo.frame.origin.y+25, SCREEN_WIDTH-40, 1)];
    ...
    if ([_MoneyField.text doubleValue] <= 0) {
        ...
        if (_useCard == YES) {
            ...
            if (card < 0) {
                ...
                VC.card = [NSString stringWithFormat:@"%f",card];
            }
        }
        else{
            VC.card = @"0.0";
