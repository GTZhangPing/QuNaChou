...
@interface MACurrentViewController ()

@property (strong, nonatomic) IBOutlet UITableView *tableview;

@property (weak, nonatomic) IBOutlet UILabel *huoqizichan;
...
@implementation MACurrentViewController
...
- (void)Requestnetwork{
    ...
    NSString *baseUrl = [NSString stringWithFormat:@"%@account/hqb.html",QNCURL];
    NSString *page = [NSString stringWithFormat:@"%ld",(long)_pageNo ];
    NSDictionary *dict = @{@"rid":rid,
                           @"uid":uid,
                           @"page":page
                           };
    AFHTTPRequestOperationManager *manager = [AFHTTPRequestOperationManager manager];     [manager.requestSerializer setHTTPShouldHandleCookies:YES];
    ...
    [manager POST:baseUrl parameters:dict success:^(AFHTTPRequestOperation * _Nonnull operation, id  _Nonnull responseObject) {
        ...
        _huoqizichan.text = [dic objectForKey:@"huoqizichan"];
        ...
        if (![infoArray isKindOfClass:[NSNull class]]) {
            for (NSDictionary *dic in infoArray) {
                ...
                MACurrentInfoModel *model = [[MACurrentInfoModel alloc] init];
...
{
    static NSString *cellIndentifier = @"MACurrentCell";
    MACurrentTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIndentifier];
    if (!cell) {
        cell = [[MACurrentTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIndentifier];
    }
    if (_arrayData && _arrayData.count>indexPath.row) {
        MACurrentInfoModel *model = self.arrayData[indexPath.row];
...
{
    CurrentInfoViewController *VC = [self.storyboard instantiateViewControllerWithIdentifier:@"CurrentInfoViewController"];
    MACurrentInfoModel *model = [self.arrayData objectAtIndex:indexPath.row];
